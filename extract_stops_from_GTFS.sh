...
echo "##### on récupère le mode GTFS de la ligne #####"
xsv join route_id temp_stop_with_routes.csv route_id routes.txt |xsv select stop_id,route_type|xsv sort|uniq > temp_stop_with_mode_.csv

echo "##### on récupère le mode de la ligne #####"
xsv join route_type temp_stop_with_mode_.csv GTFS_route_type ../GTFS_transport_modes.csv |xsv select stop_id,transport_mode_name > temp_stop_with_mode.csv

echo "##### on récupère les infos de l'arrêt #####"
xsv join stop_id stops.txt stop_id temp_stop_with_mode.csv |xsv select 'stop_id[0],stop_lat,stop_lon,stop_name,transport_mode_name' > temp_stop_full.csv
